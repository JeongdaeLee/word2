...
function travelbee_customize_register_general( $wp_customize ){
    ...
    if( travelbee_is_btif_activated() ){
        ...
        $wp_customize->add_control(
            new Travelbee_Note_Control( 
    			...
    			array(
    				'section'	  => 'instagram_settings',
    				'description' => sprintf( __( 'You can change the setting BlossomThemes Social Feed %1$sfrom here%2$s.', 'travelbee' ), '<a href="' . esc_url( admin_url( 'admin.php?page=class-blossomthemes-instagram-feed-admin.php' ) ) . '" target="_blank">', '</a>' )
    			)
    		)
        );        
    }else{
		...
		$wp_customize->add_control(
			new Travelbee_Plugin_Recommend_Control(
				...
				array(
					...
					'description' => sprintf( __( 'Please install and activate the recommended plugin %1$sBlossomThemes Social Feed%2$s. After that option related with this section will be visible.', 'travelbee' ), '<strong>', '</strong>' ),
				)
			)
		);
    }
